...
class Parser:
    ...
    def expect(self, kind: str, message: str | None = None) -> Token:
        token = self.current()
        if token.kind != kind:
            raise ParseError(message or f"Expected {kind}, got {token.kind} ({token.value!r}) at {token.line}:{token.column}")
        return self.advance()

    def span_from(self, start: Token, end: Token) -> SourceSpan:
        return SourceSpan(line=start.line, column=start.column, end_line=end.line, end_column=end.column)

    def parse(self) -> Module:
        module = Module()
        if self.match("MODULE"):
            name_token = self.expect("IDENT")
            module.name = name_token.value
            module.span = SourceSpan(name_token.line, name_token.column, name_token.line, name_token.column)
        ...
        while self.current().kind != "EOF":
            ...
            if kind == "IMPORT":
                ...
                module.exports.append(self.parse_export())
            elif kind == "TYPE":
    ...
    def parse_import(self) -> ImportDecl:
        ...
        return ImportDecl(module_name=module_name, alias=alias, span=self.span_from(start, self.tokens[self.index - 1]))

    def parse_export(self) -> ExportDecl:
        start = self.expect("EXPORT")
        name = self.expect("IDENT").value
        return ExportDecl(name=name, span=self.span_from(start, self.tokens[self.index - 1]))
